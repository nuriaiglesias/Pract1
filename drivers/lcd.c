...
struct Structure My_Div2(int a, int b)
{
  int c = 0;
  int r = 0;
  asm(
    ...
    "b start_div\n"
    "loop_div:\n\t"
    "sub %[MINUENDO], %[MINUENDO], %[SUSTRAENDO]\n\t" 
    "add %[COCIENTE], %[COCIENTE], #1\n" 
    "start_div:\n\t"
    "cmp %[MINUENDO], %[SUSTRAENDO]\n\t" 
    "bge loop_div\n\t"
    "mov %[COCIENTE], %[COCIENTE]\n\t" 
    "mov %[RESTO], %[MINUENDO]\n\t"
    : [MINUENDO] "+l" (a), [COCIENTE] "+l" (c), [RESTO] "=l" (r)
    : [SUSTRAENDO] "l" (b)
  );
  struct Structure e1 = {c,r};
...
void lcd_display_dec(uint16_t value)
{
  if (value > 9999) {
    ...
    struct Structure thousands = My_Div2(value, 1000);
    struct Structure hundreds = My_Div2(value - thousands.c*1000, 100);
    struct Structure tens = My_Div2(value - thousands.c*1000 - hundreds.c*100, 10);
    struct Structure ones = My_Div2(value - thousands.c*1000 - hundreds.c*100 - tens.c*10, 1);

    lcd_set(thousands.c, 1);
